...
async function collect() {
  ...
  // Bei Merge-keep-last (Quelle ausgefallen) bleibt der ALTE Zeitstempel erhalten -> echtes Alter sichtbar.
  for (const k of Object.keys(LIVE)) {
    if (k.endsWith("_conf") || k.endsWith("_ts") || k === "updated") continue;
    LIVE[k + "_ts"] = (LIVE[k] === null || LIVE[k] === undefined) ? null : LIVE.updated;
  }
  return LIVE;
}
      ...
      for (const k in old) {
        ...
        if (merged[k] === null || merged[k] === undefined) {
          if (old[k] !== null && old[k] !== undefined) {
            ...
            if (old[k + "_ts"] !== undefined) merged[k + "_ts"] = old[k + "_ts"];
