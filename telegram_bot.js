...
(async () => {
  ...
  // Befehlsmenü einmalig registrieren (erscheint beim Tippen von "/")
  if (!state.commands_registered) {
    await tgSetCommands();
    state.commands_registered = true;
    console.log("Telegram-Befehlsmenü registriert.");
  }

  const updates = await tgGetUpdates(state.offset ? state.offset + 1 : 0);

  if (updates.length === 0) { console.log("Keine neuen Nachrichten."); return; }
